...
from app.database.chat_history import get_chat_history
from app.database.promotion_slots import get_or_create_state

from app.core.config import settings
from app.mock.chat import *

async def stream_agent(chat_id, message):
    slots = get_or_create_state(chat_id)
    state = AgentState(
        history=get_chat_history(chat_id=chat_id), 
        promotion_slots=PromotionSlots(**slots),
        sql_context={"conn_str":settings.CONN_STR, "schema_info":settings.SCHEMA_INFO},
