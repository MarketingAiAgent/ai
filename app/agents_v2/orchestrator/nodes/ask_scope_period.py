...
def ask_scope_period_node(state: AgentState) -> AgentState:
    ...
    if not state.promotion_slots:
        return state.model_copy(update={"response": "브랜드 기준과 제품 기준 중 어느 쪽으로 진행하시고, 기간은 어느 정도로 보실까요?"})
    ...
    if missing == ["scope"]:
        return state.model_copy(update={"response": "브랜드 기준과 제품 기준 중 어느 쪽으로 진행하시겠습니까?"})
    elif missing == ["period"]:
        return state.model_copy(update={"response": "기간은 어느 정도로 계획하고 계신가요?"})
    else:
        return state.model_copy(update={"response": "브랜드 기준과 제품 기준 중 어느 쪽으로 진행하시고, 기간은 어느 정도로 보실까요?"})